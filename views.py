...
try:
    format_list.extend(
        [(i[0], i[0]) for i in Format.objects.values_list('name')]
    )
    geom_type_list.extend(
        [(i[0], i[0]) for i in Geom_type.objects.values_list('name')]
    )
...
class SearchForm(forms.Form):
    ...
    geom_types = forms.MultipleChoiceField(
        required=False,
    ...
    min_records = forms.IntegerField(
        required=False,
    ...
    search_within_selected = forms.BooleanField(
        required=False,
...
def check_origin(request):
    ...
    if (
        not '131.212.123' in request.META.get('REMOTE_ADDR', '')
...
def search(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if d['path_txt']:
                ...
                selection = selection.filter(q)
                filters.add(
                    "Path contains %s" % ' or '.join(d['path_txt'].split(','))
                )
            ...
            if d['geom_types']:
                ...
                selection = selection.filter(q)
                filters.add(
                    "Geometry type is %s" % ' or '.join(d['geom_types'])
                )
            ...
            request.session['selected'] = [
                ...
                or not d['search_within_selected']
                or i[0] in prev_selection
            ]

            selected_count = len(request.session['selected'])

            selected = [
                [i[0], translate_path(i[1]), i[1], i[2]]
    ...
    return render_to_response(
        "gis_asset/search.html", context, RequestContext(request),
    )
